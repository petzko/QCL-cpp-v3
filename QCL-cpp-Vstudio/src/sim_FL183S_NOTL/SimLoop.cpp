...
void init_matlab(MB::SimSettings& set, MB::SimData& dat){
	dat.m_Engine = engOpen(NULL);
	dat.U_x = mxCreateDoubleMatrix(1,set.N,mxCOMPLEX);
	dat.V_x = mxCreateDoubleMatrix(1,set.N,mxCOMPLEX);
}
#endif
...
#ifdef _DISP_COMPENSATE_ //TODO: Use library routines to obtain YMOD => SUBSEQUENT FFTW
	if (engEvalString(dat.m_Engine,"load('disp_data.mat');") == 0){ 
		std::cout << "MATLAB cannot load dispersion data! \n";
	}else{
		std::cout << "DISPERSION DATA SUCCESSFULLY LOADED! \n";
	} // load the dispersion data variable! 

	if (engEvalString(dat.m_Engine,"plot(fftshift(angle(Ymod))); getframe;")==0){
		std::cout << "MATLAB cannot plot dispersion data! \n";
	}else{
		std::cout << "DISPERSION DATA SUCCESSFULLY PLOTTED! \n";
	} // plot the dispersion data variable! 
#endif 
}
...
void makeBlochVars(MB::SimSettings& set, MB::SimData& dat){
	...
	for(int i = 0 ; i < set.N; i++ ){
		...
		//dat.SD0[dat.r130][i] = 1e-15/dat.trace_rho;
		//dat.SD0[dat.n32p][i] = 1e-15/dat.trace_rho;
		//dat.SD0[dat.n32m][i] = 1e-15/dat.trace_rho;
		//dat.SD0[dat.n12p][i] = 1e-15/dat.trace_rho;
		//dat.SD0[dat.n12m][i] = 1e-15/dat.trace_rho;
...
void updateSolutions(MB::SimSettings& set, MB::SimData & dat){
	copyData(dat.SD1[0],dat.SD0[0],dat.Nvars*set.N);
}
void stepMaxwellVars(MB::SimSettings&  set, MB::SimData& dat){
	...
	for(int x = 0 ; x < set.N ;x++){
		*(mxGetPr(dat.U_x)+x) = REAL(dat.SD1[dat.U][x]);
		*(mxGetPi(dat.U_x)+x) = IMAG(dat.SD1[dat.U][x]);

		*(mxGetPr(dat.V_x)+x) = REAL(dat.SD1[dat.V][x]);
		*(mxGetPi(dat.V_x)+x) = IMAG(dat.SD1[dat.V][x]);
	}
	engPutVariable(dat.m_Engine,"U",dat.U_x);
	engPutVariable(dat.m_Engine,"V",dat.V_x);
	char comm [512];
	sprintf(comm,"U_lin = ifft(fft([U(1:end-1),V(end:-1:2)]).*Ymod ); U = U_lin(1:%d);  V = [U(1), U_lin(end:-1:%d)];",set.N,set.N);
	engEvalString(dat.m_Engine,comm); // can be done much faster with FFTW! 
	mxArray* U_comp = engGetVariable(dat.m_Engine, "U");
	mxArray* V_comp = engGetVariable(dat.m_Engine, "V");

#pragma omp parallel for
	for(int x =0;x<set.N;x++){
		dat.SD1[dat.U][x] = *(mxGetPr(U_comp)+x)+_i* (*(mxGetPi(U_comp)+x));
		dat.SD1[dat.V][x] = *(mxGetPr(V_comp)+x)+_i* (*(mxGetPi(V_comp)+x));
	}

	
#ifdef _MATLAB_PLOT_
		if(dat.iter_ctr% 100 == 0){
			engEvalString(dat.m_Engine,"plot(abs(U).^2+abs(V).^2); getframe;");
		}
#endif
...
void startSim(char* simFile,char* setFile,double* deph_times,int attempt){
	...
	*/
	...
	copyData(dat.SD0[0],dat.SD1[0],dat.Nvars*set.N);
	...
	char fname[512];
	sprintf_s(fname,"sim_res%d.mat",attempt);

#ifdef _MATLAB_SUPPORT_

	write2Matlab(record_U,"record_U",fname); mxDestroyArray(record_U); 
	write2Matlab(record_V,"record_V",fname); mxDestroyArray(record_V); 

	write2Matlab(record_INJ,"record_pINJ",fname);	mxDestroyArray(record_INJ); 
	write2Matlab(record_ULL,"record_pULL",fname);	mxDestroyArray(record_ULL); 
	write2Matlab(record_LLL,"record_pLLL",fname); mxDestroyArray(record_LLL); 
	write2Matlab(record_POP1,"record_pPOP1",fname); mxDestroyArray(record_POP1); 
	write2Matlab(record_POP2,"record_pPOP2",fname); mxDestroyArray(record_POP2); 
	mxArray *dt = mxCreateDoubleScalar(dat.dt);
	write2Matlab(dt,"dt",fname); mxDestroyArray(dt); 
	mxArray *N_t = mxCreateDoubleScalar(dat.N_t);
	write2Matlab(N_t,"N_t",fname);mxDestroyArray(N_t); 
	mxArray *E0 = mxCreateDoubleScalar(dat.E0);
	write2Matlab(E0,"E0",fname); 	mxDestroyArray(E0); 

	mxArray *T_R = mxCreateDoubleScalar(dat.T_R);
	write2Matlab(T_R,"T_R",fname);	mxDestroyArray(T_R);

	mxArray *O13 = mxCreateDoubleScalar(dat.HTB[dat.INJ][dat.ULL]);
	write2Matlab(O13,"O13",fname);	mxDestroyArray(O13); 
